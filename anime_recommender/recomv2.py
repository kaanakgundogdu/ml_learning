...
# mlb = MultiLabelBinarizer()
# genre_encoded = mlb.fit_transform(df['genres'])
# genre_df = pd.DataFrame(genre_encoded, columns=mlb.classes_, index=df.index) # Keep index

# # 2. Normalize Average Score
# scaler = MinMaxScaler()
# normalized_score = scaler.fit_transform(df[['averageScore']])
# score_df = pd.DataFrame(normalized_score, columns=['normalized_score'], index=df.index) # Keep index


# # 3. Process Descriptions with TF-IDF (Keeping for description info, but can be replaced later if needed)
# tfidf = TfidfVectorizer(stop_words='english', max_features=5000)
# tfidf_matrix = tfidf.fit_transform(df['description'])
# tfidf_df = pd.DataFrame(tfidf_matrix.toarray(), index=df.index) # Keep index


# # 4. Concatenate Features
# feature_df = pd.concat([tfidf_df, genre_df, score_df], axis=1) # Concatenate along columns, keeping index
# feature_matrix = feature_df.values # Convert to numpy array
# feature_tensor = torch.tensor(feature_matrix, dtype=torch.float32).to(device)


# # 5. Define Autoencoder Model (Adjusted Input Dimension)
# class AnimeAutoencoderCombined(nn.Module): # Renamed class
#     def __init__(self, input_dim, encoding_dim):
#         super(AnimeAutoencoderCombined, self).__init__() # Renamed class here too
#         self.encoder = nn.Sequential(
#             nn.Linear(input_dim, encoding_dim),
#             nn.ReLU(),
#             nn.Linear(encoding_dim, encoding_dim // 2), # Added a layer
#             nn.ReLU()
#         )
#         self.decoder = nn.Sequential(
#             nn.Linear(encoding_dim // 2, encoding_dim), # Adjusted layer size
#             nn.ReLU(),
...
# # 6. Initialize Model, DataLoader, Loss, Optimizer (Adjusted Input Dimension)
# input_dim = feature_matrix.shape[1] # Adjusted input dimension
# encoding_dim = 128 # Increased encoding dimension slightly

# model = AnimeAutoencoderCombined(input_dim, encoding_dim).to(device) # Use Combined Autoencoder
# dataset = TensorDataset(feature_tensor, feature_tensor)
# dataloader = DataLoader(dataset, batch_size=32, shuffle=True)
# criterion = nn.MSELoss()
...
# # 8. Obtain Encoded Representations
# with torch.no_grad():
#     encoded_representations, _ = model(feature_tensor)


# # 9. Compute Cosine Similarity in Latent Space
# latent_cosine_sim = cosine_similarity(encoded_representations.cpu().numpy())

# # 10. Recommendation Function (Latent Features - No changes needed here as logic is same)
...
#     recommended_series = {}  # Dictionary to track recommended series, key=series_id, value=title
#     recommendations = []
#     count = 0

#     input_title_lower = title.lower() # Lowercase input title for series ID extraction

#     def get_series_identifier(anime_title): # Function to extract series identifier
#         title_lower = anime_title.lower()
#         words = title_lower.split()
#         if "digimon" in title_lower: # Special case for Digimon (can be expanded for others)
#             return "digimon"
#         elif "pokemon" in title_lower or "pocket monsters" in title_lower: # Special case for Pokemon
#             return "pokemon"
#         elif len(words) >= 2:
#             return " ".join(words[:2]) # Use first 2 words as series identifier
#         elif words:
#             return words[0] # If only one word, use that
#         else:
#             return anime_title.lower() # Fallback: use the whole lowercased title


#     input_series_id = get_series_identifier(title) # Get series ID for the input title


#     for i in range(1, len(sim_scores)): # Start from 1 to exclude the input title itself
#         sim_index = sim_scores[i][0]
#         recommended_title = df['title_romaji'].iloc[sim_index]
#         recommended_series_id = get_series_identifier(recommended_title)


#         if recommended_series_id != input_series_id: # Exclude series similar to the input series itself.
#             if recommended_series_id not in recommended_series: # Check if series already recommended
#                 recommended_series[recommended_series_id] = recommended_title # Add series to recommended series dict
#                 recommendations.append(recommended_title) # Add title to recommendations list
#                 count += 1
#                 if count >= 10: # Stop after getting 10 unique series recommendations
#                     break

#     return pd.Series(recommendations) # Return as Pandas Series

# # 11. Example Usage and Output
# latent_recommendations = get_recommendations_latent('Naruto') 
